## 1. 오류와의 씨름
### 1.1 도커는 별도의 자체 폴더구조를 활용하는 특징을 몰라서 맥운영체제에 존재하는 디렉토리 경로를 도커파일에 입력하는 바람에 도커이미지 빌드에 실패하였습니다.
...
```
##
### 1.2 dockerfile에 사소한 오타가 있어 도커 이미지 빌드에 실패했습니다.
...
```
##
### 1.3 도커 컨테이너 내부에서 파이선 파일을 실행하려면 교육생에게 권한이 허가된 디렉토리에 도커파일, 파이썬 파일이 모두 존재하야되는지 몰라서 파이썬 도커이미지 실행을 못했습니다.
```
